...
private:
    ...
    void createTextureImage()
    {
        ...
        // one last transition to prepare texture image for shader access
    ...
    void createImage(uint32_t texWidth, uint32_t texHeight, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
    {
        ...
        imageInfo.format = VK_FORMAT_R8G8B8A8_SRGB; // use same format for texels als pixels in buffer
        imageInfo.tiling = VK_IMAGE_TILING_OPTIMAL; // texels are laid out in implementation defined order for optimal access
        imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // not usable by GPU and very first transition will discard texels
		imageInfo.usage = VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT; // image will be used as destination for copy operation and as a we want to access image from shader
